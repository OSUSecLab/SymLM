...
@register_model('roberta_mf_nau')
class RobertaModelMFNAU(FairseqEncoderModel):
    ...
    def register_classification_list(self, name, num_classes=None, pooling_option=None, topK=3, inner_dim=None, **kwargs):
        ...
        if name == 'funcname_head':
            self.classification_heads[name] = FuncNameHead(
        ...
        elif name == 'cls_multi_class':
            self.classification_heads[name] = CLSMultiLabel(
                input_dim=self.args.encoder_embed_dim,
                activation_fn=self.args.pooler_activation_fn,
                num_classes=num_classes,
                pooler_dropout=self.args.pooler_dropout,
        ...
        elif name == 'func_name_multi_class':
            self.classification_heads[name] = FuncNameMultiClass(
                input_dim=self.args.encoder_embed_dim,
                activation_fn=self.args.pooler_activation_fn,
                num_classes=num_classes,
                pooler_dropout=self.args.pooler_dropout,
        ...
        elif name == 'func_name_pooling':
            self.classification_heads[name] = FuncNamePooling(
                input_dim=self.args.encoder_embed_dim,
...
class RobertaLMHeadReg(nn.Module):
    """Head for masked language modeling as regression task."""

    def __init__(self, embed_dim, activation_fn):
        super().__init__()
        self.dense = nn.Linear(embed_dim, 1024)
        self.activation_fn = utils.get_activation_fn(activation_fn)
        self.layer_norm = LayerNorm(1024)
        self.weight = nn.Linear(1024, 1, bias=False).weight
...
@register_model_architecture('roberta_mf_nau', 'roberta_mf_nau')
...
@register_model_architecture('roberta_mf_nau', 'roberta_mf_nau_base')
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture('roberta_mf_nau', 'roberta_mf_nau_large')
...
@register_model_architecture('roberta_mf_nau', 'xlm_mf_nau')
