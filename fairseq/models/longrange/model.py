...
from fairseq.modules.linear_attention_transformer import LinearAttentionTransformerLMMultifield
...
@register_model('longrange')
class LongrangeModel(FairseqEncoderModel):
    ...
    def add_args(parser):
        ...
        # efficient transformer parameters
...
class LongrangeEncoder(FairseqEncoder):
    """Longrange encoder.
    It can be multiple encoder candidate including
    reformer, sinkhorn, routing, linformer, etc."""
    ...
    def __init__(self, args, dictionary):
        ...
        self.sentence_encoder = LinearAttentionTransformerLMMultifield(
            num_tokens={field: len(dictionary[field]) for field in dictionary},
            dim=args.encoder_embed_dim,
            depth=args.encoder_layers,
            max_seq_len=args.max_positions,
            heads=args.encoder_attention_heads,
            ff_dropout=args.dropout,
            attn_layer_dropout=args.attention_dropout,
            attn_dropout=args.activation_dropout,
            emb_dim=128,
            dim_head=128,
            blindspot_size=64,
...
@register_model_architecture('longrange', 'longrange')
...
@register_model_architecture('longrange', 'longrange_base')
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture('longrange', 'longrange_large')
