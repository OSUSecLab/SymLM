...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        # new_order = [0, 0, 0, 0, 1, 1, 1, 1, ..., bsz-1, bsz-1, bsz-1, bsz-1]. repeat every elements of range(bsz) by
        # beam_size times
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        # a = new_order.cpu().detach().numpy()
        new_order = new_order.to(src_tokens.device).long()
        # a = encoder_outs[0]['final_hidden'][0].cpu().detach().numpy()

        # with this reorder method, the encoder_outs[0]['final_hidden']'s batches will be repeated by beam_size times.
        # For example, original encoder_outs[0]['final_hidden'] = [batch1, batch2, ...]
        # After execution: encoder_outs[0]['final_hidden'] = [batch1, batch1, batch1, batch1, batch2, batch2, batch2, batch2,...]
        encoder_outs = self.model.reorder_encoder_out(encoder_outs, new_order)
        # b = encoder_outs[0]['final_hidden'][0].cpu().detach().numpy()
