...
@register_task('cls_multi_class')
class ClsMultiClass(LegacyFairseqTask):
        ...
        for field in self.fields:
            ...
            src_tokens[field] = PrependTokenDataset(
                RightPadDataset(
                    StripTokenDataset(
                        TruncateDataset(
                            src_dataset, self.args.max_positions),
        ...
        dataset.update(
            target=RightPadDataset(
                # OffsetTokensDataset(
                    ...
                    # offset=-self.label_dictionary.nspecial,
                # ),
                # pad_idx=self.label_dictionary.pad() - self.label_dictionary.nspecial + 1
                pad_idx=self.label_dictionary.pad()
            )
        )
    ...
    def build_model(self, args):
        ...
        # a = getattr(args, 'classification_head_name', 'cls_multi_class')

        # TODO: classification head
        model.register_classification_list(
            # 'cls_multi_class',
            getattr(args, 'classification_head_name', 'cls_multi_class'),
